...
#as the plot may shows ,use ma_5 of the bull line instead of bull line ,and when the close price
lower a little than the ma(bull)then means the index out of bull is more usefull
"""
...
from utils import pyalg_utils,formular,BBcurve2
import pandas as pd
...
def kdj(param = [14,3,0,3,0,70,30]):
    ...
    dat = pd.read_csv("000002.csv",encoding='gbk',parse_dates=[0],index_col=0)
    ...
    myStrategy = kdj_backtest(feed, 'orcl',param = param,df=BBcurve2.BBcurve(dat))  #set the param of STOCH and crossover

...
def getShData():
    ...
    return dat
def BBcurve(data=getShData()[2000:2330],T=5,n=20,alpha=0.4,resample=False):
    """
    T = 5                        # 采样的间隔交易日
    n = 20                       # 采样点数
    ...
    plt.plot(bb.index.values,bb.bull.values,color='r',label="$Bull$",linewidth=2)  #the first one is n*T+T,and the bull_c is for n*T+T
